...
- 敏感信息只允许放在后端环境变量或 `.env`，不得进入前端代码、浏览器响应、日志原文或文档正文。
- 当前私有仓库明确允许 `.env` 随仓库提交，用于换机后直接恢复运行配置；仓库必须保持私有。
- `.env.example` 只允许放占位符、配置项说明和示例结构。
- `data/app.sqlite`、`data/dashboard-cache.json` 不得提交；换机时通过备份或同步恢复，见 [Git 与本地数据规范](../handbook/git-and-data.md)。
- 若仓库未来公开、迁移到非受控平台或共享给无权限成员，必须先从历史中移除 `.env` 并轮换所有密钥。
...
- 检查日志脱敏测试通过。
- 确认 `.env` 只存在于私有仓库后端运行配置范围内，未进入前端代码、日志或文档正文。
- 确认 `data/app.sqlite`、`data/dashboard-cache.json`、`data/precomputed/`、`data/read-model/` 未被 Git 跟踪。
...
- token 出现在前端代码中。
- token 出现在浏览器请求中。
- AppSecret 出现在前端代码、文档正文、日志或对外响应中。
- 后端新增了钉钉写入接口。
- 本地编辑接口绕过审计直接改最终数据。
- 同步接口没有任何鉴权。
