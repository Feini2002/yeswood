...
- 产品清单字段保留记录，理解为外部产品清单发出/接收事实；它会进入统一阶段事实和首页采购推进分组，但不计入设计责任制完成口径。
...
| `productListVolume` | 公司协同 | `产品清单发出时间`、`产品清单接收时间`、`流程记录：产品清单接收时间` | 产品清单发出/接收记录在本月 |
| `schemeVolume` | 软装 | `软装方案开始时间` | 方案开始时间在本月，不要求先有产品清单 |
| `purchaseVolume` | 公司协同 | `采购时间` | 采购时间在本月，表示采购推进 / 采购中，不等同采购完成 |
| `siteVolume` | 公司协同 | `软装项目进度`、`摆场开始时间`、`摆场时间`、`现场摆场时间`、`摆场文件发出时间(项目群）`、`摆场文件发出时间（项目群）` | 摆场开始日期在本月计入进行中；摆场文件发出日期在本月计入完成；进度含「摆场 / 闭环」只作辅助证据 |
