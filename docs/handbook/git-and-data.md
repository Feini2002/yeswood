...
1. `git clone` 得到代码与规则 JSON、人员种子。
2. 复制本机备份的 `.env` 与 `data/app.sqlite`，或在新环境配置 `.env` 后执行同步灌库。
...
1. 确认 `git status` 中**没有** `data/app.sqlite`、`data/dashboard-cache.json`、`.env`。
2. `npm test` 通过。
...
6. 确认 clone 后按上一节恢复 `.env` 与数据库即可运行。
