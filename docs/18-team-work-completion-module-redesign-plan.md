...
- 公司层面的项目闭环只认硬装、软装都闭环；睡眠店等特殊项目按既有硬装单轨规则处理。
...
| 项目总闭环情况 | `lifecycle` | 公司层面项目闭环，常规项目硬装 + 软装都闭环 |
...
- 软装项目进度、点位状态、摆场状态或展示相关字段明确为完成；
- 典型状态包括点位已完成、摆场完成、软装完成等。
...
- 常规项目：硬装项目进度精确闭环，并且软装项目进度精确闭环。
- 睡眠店 / 仅硬装特殊项目：按既有硬装单轨闭环规则。
- 如果项目只有设计责任闭环，但硬装或软装流程未闭环，则当前模块不算项目总闭环。

进行中判断：

- 硬装或软装任一轨道已经启动但尚未双轨闭环，可算项目闭环进行中；
- 暂停、停止、作废项目不算进行中；
- 已闭环项目不再算进行中。
...
摆场完成当前根据软装、点位、摆场相关字段判断。后续需要与钉钉表单维护同事确认：
...
双轨闭环项目建议使用硬装、软装中较晚的闭环日期作为项目总闭环日期。若公司未来要求按某个“项目最终闭环时间”字段统计，可以替换日期读取层，但不应改动 UI 和聚合结构。
