...
- **团队工作完成情况**：小组页主模块，按团队 / 小组 / 成员聚合月度完成量与进行中状态；摘要和项目明细拆分为读模型优先、必要时计算兜底的加载路径（`public/pages/team-work-completion.mjs`）。
- 项目明细弹窗、项目 drill-down 列表、今日处理动作队列。
- 统一项目阶段提醒口径：项目详情、下钻列表、小组待处理队列、生命周期分组和读模型共用同一阶段提醒表；读模型写入 compact `stageReminder` / `workflowFacts`，摆场开始即进入“摆场中”，摆场文件发出即默认“摆场结束”。
...
- 看板加载性能优化：项目目录 `view=summary` 缓存、下钻 `fields=ids` 拼装、小组指标与团队完成度摘要/明细预计算、snapshot 签名缓存。
...
| `src/backend/readModelRepository.mjs` | 小组指标、团队完成度与项目明细读模型读取 |
...
| `GET` | `/api/team-work-completion` | 团队工作完成情况；读模型优先，支持摘要/明细与计算兜底 |
